...
cdc %>% 
            ...
            select(Chapter, State, Year, N_Crude_Rate, S_Crude_Rate) %>% 
            filter(Chapter == "Certain infectious and parasitic diseases", State == "AL") %>% 
            ggplot() +
...
ui <- fluidPage(
    ...
    sidebarLayout( 
      sidebarPanel(
        selectInput("choose state", label = strong("State"), 
                    choices = levels(as.factor(cdc$State)), 
                    selected = 1),
    
        selectInput("Choose Cause of Death", label = strong("Cause of Death"), 
    ...
    output$distPlot <- renderPlot({
        cdc %>% 
            ...
            mutate(S_Count=sum(Deaths),
                   S_Crude_Rate=10^5*(S_Count/Population)) %>%
            select(Chapter, State, Year, N_Crude_Rate, S_Crude_Rate) %>% 
            filter(Chapter == input$select2, State == input$select1) %>% 
            ggplot() +
            geom_bar(aes(x = Year, weight = S_Crude_Rate)) +
            labs(x = "State", y = "Crude Mortality Rate") + 
            geom_line(aes(x = Year, y = N_Crude_Rate, linetype = "National Average"), col = "red", lwd = 1) +
